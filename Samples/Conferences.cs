...
namespace OMSamples.Samples
{
    ...
    class Conferences : ISample
    {
        ...
        {
            return map.Select(x => gw.CreateGatewayParameterBinding(ps.GetGatewayParameterByName(x.Key), ps.GetGatewayParameterValueByName(x.Value))).ToArray();
        }
        void WebMeetingBridgeUpdate()
        {
            using (var lines = ps.GetExternalLines().GetDisposer(x => x.GetPropertyValue("WEBRTC_ACTIVE_BRIDGE") == "1"))
            {
        ...
        {
            this.ps = ps;
            using (var confstates = ps.InitializeStatistics("S_CONFERENCESTATE").GetDisposer())
            using (var schedconf = ps.InitializeStatistics("S_SCHEDULEDCONF").GetDisposer())
            {
                ...
                switch (command)
                {
                    ...
                    case "scheduled":
                        ...
                        {
                            var conf = ps.GetByID(command == "active" ? "S_CONFERENCESTATE" : "S_SCHEDULEDCONF", id);
                            PrintStat(conf);
                            int schedule_id = 0;
                            if (command == "active")
                            {
                    ...
                    //case "joinaudio":
                    //    {
                    //        var stat = ps.GetByID("S_SCHEDULEDCONF", id);
                    //        if (stat["pv_meeting_type"] == "1")
                    //        {
                    //            var newconf = ps.CreateStatistics("S_SCHEDULEDCONF", new Random((int)DateTime.Now.Ticks).Next(100000, 999999).ToString());
                    ...
                    case "removeschedule":
                        {
                            using (var schedule = ps.GetByID("S_SCHEDULEDCONF", id))
                            {
                                schedule.Delete();
                            }
                        }
                    ...
                    default:
                        {
                            using (var confExt = ps.GetAll<ConferencePlaceExtension>().GetDisposer().First())
                            {
                                ...
                                switch (command)
                                {
                                    ...
                                    case "add": // call to number which is stored in 
                                        {
                                            using (var activeconf = ps.GetByID("S_CONFERENCESTATE", id))
                                            {
                                ...
                                ps.ServiceCall(confExt.Number, dict);
                            }
                        }
                        break;
                }
